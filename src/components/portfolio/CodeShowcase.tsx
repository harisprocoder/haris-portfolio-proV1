...
export default function CodeShowcase() {
  ...
  return (
    <section ref={sectionRef} style={{ background: "#12100C" }}>
      <div className="max-w-[1200px] mx-auto px-5 sm:px-6">
        ...
        >
          ...
          <h2
            className="font-['Space_Grotesk'] text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-3 sm:mb-4"
            style={{ color: "#F5EFE6", letterSpacing: "-0.02em" }}
          >
            Clean, modern <span className="gradient-text">code quality</span>
          </h2>
          <p className="text-sm sm:text-base mb-10 sm:mb-12 max-w-lg" style={{ color: "#A89F8F" }}>
        ...
        <div className="grid lg:grid-cols-5 gap-6 sm:gap-8">
          ...
          <motion.div
            ...
            style={{
              ...
              boxShadow: "0 25px 50px rgba(0,0,0,0.4), 0 0 40px rgba(255,132,0,0.04)",
            }}
          ...
          >
            ...
            <div className="flex min-h-[300px] md:min-h-[440px]">
              ...
              >
                <p className="px-3 mb-2 text-[10px] uppercase tracking-wider font-semibold" style={{ color: "#45475a" }}>Explorer</p>
                {fileTree.map((f) => (
                  <div
                    key={f.name}
                    ...
                    style={{
                      ...
                      background: f.active ? "rgba(255,132,0,0.08)" : undefined,
                    }}
                  >
                    <i
                      className={`${f.isDir ? "fas fa-folder" : "fas fa-file-code"} text-[10px]`}
                      style={{ color: f.isDir ? "#f9e2af" : f.active ? "#FF8400" : "#585b70" }}
                      aria-hidden="true"
                    />
                    {f.name}
                  </div>
              ...
              <div className="flex-1 overflow-x-auto">
                <div className="p-4">
                  {codeLines.map((line, i) => (
                    ...
                    >
                      <span className="w-8 text-right mr-4 shrink-0 select-none text-[11px]" style={{ color: "#45475a" }}>{i + 1}</span>
                      <span className="whitespace-pre">{highlightLine(line.text, line.type)}</span>
                    </motion.div>
                  ))}
                  {isInView && (
                    <motion.div
                    ...
                    >
                      <span className="w-8 text-right mr-4 shrink-0 select-none text-[11px]" style={{ color: "#45475a" }}>{codeLines.length + 1}</span>
                      <span className="inline-block w-[7px] h-[16px] mt-[3px] animate-pulse" style={{ background: "#FF8400" }} />
                    </motion.div>
                  )}
                </div>
              </div>
            ...
            {features.map((f) => (
              <motion.div
                ...
                whileHover={{
                  ...
                  boxShadow: "0 0 30px rgba(255,132,0,0.06)",
                  transition: { duration: 0.3 },
                }}
              >
                <div className="text-2xl mb-3" aria-hidden="true">{f.icon}</div>
                <h3 className="font-['Space_Grotesk'] font-bold text-sm sm:text-base mb-2" style={{ color: "#F5EFE6" }}>{f.title}</h3>
                <p className="text-xs sm:text-sm leading-relaxed" style={{ color: "#A89F8F" }}>{f.desc}</p>
              </motion.div>
            ))}
          </motion.div>
        </div>
